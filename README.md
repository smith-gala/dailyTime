...
飞书发起（任意手机聊天App发起） · AI 分析 · 影视素材检索 · 竖屏渲染 · 人工审核
...
Daily Time 是一个面向英语学习内容创作的短视频自动化项目。随便给它一句英文（这个可以使用数据库，后面会发布的），系统会分析中文释义与音标，从 PlayPhrase 获取影视片段，合成为统一风格的 **1080 × 1920 竖屏视频**，再通过飞书把成片送回给你审核。

它把选句之后的重复工作串成了一条完整流程：内容分析、素材下载、字幕排版、音频处理、视频渲染和结果通知。你负责内容判断，系统负责把制作过程跑完。

## 实际账号

本项目已用于 **Listening time** 英语学习账号的内容制作，账号已在抖音和快手运营。下面是提供的实际账号截图（两周时间的结果），展示了「每日一句」的成片风格与发布效果。
...
截图中的粉丝、获赞和播放量仅代表截图时的状态。账号发布可以由人工完成也可以后续接入API定时发布；当前项目实现的是视频制作、飞书交付与审核，不包含抖音或快手自动发布。

## 一句话到一条视频

在接入飞书与 OpenClaw 后，可以这样发起任务：

> 帮我制作一句 “Time will tell.” 的每日一句视频（可以使用AI工具整理每日一句到数据库，这样能够实现只说制作视频达到目的）。
...
分析得到的讲解与例句属于内容结果；视频画面按 `config.yaml` 中的模板排版，不会把所有分析字段都堆进画面，这是本人测试之后最优的结果，因为抖音或者快手会根据手机型号改变视频宽度！！。
